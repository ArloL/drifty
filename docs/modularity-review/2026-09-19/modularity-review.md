...
It did not take recommendations 2 and 3. Those landed afterwards, in `ConfigSpelling`, by a route this analysis did not consider: the spelling is derived from the generated `Drifty.*` constant rather than from the `@JsonProperty` annotation recommendation 2 proposed. That is strictly better here. `Drifty.java` is generated from `config/drifty.pkl` at build time, so a misspelling no longer compiles and a renamed union member breaks the build, where reading the annotation would have bought the wire vocabulary — which coincides with the config's today and is not held to it — at the cost of reflection the native image needs metadata for. The eight hand-written tables and the lower-casing rule are gone, and `ConfigSpellingTest` (recommendation 3) looks each union member up by the client constant's own name, so a mis-mapped arm fails. One residual: `Pages.buildType`'s union is written inline in the schema rather than as a typealias, so codegen leaves it a `String` and its two spellings are the one pair not checked by the compiler.
