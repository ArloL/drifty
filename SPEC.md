...
**The file is `pkl format` output.** An adopter commits the export as their starting config, and a repository that checks formatting in CI — drifty-arlol runs `pkl format --diff-name-only drifty.pkl` — would otherwise fail on the first run. Two shapes are what the formatter cares about and what `PklWriter` therefore writes: an empty body is `{}` on the line that opens it, and an assignment that would pass a hundred columns puts its value on its own line, indented one level past the name. Nothing else is width-sensitive — a block header, a listing element and a long `//` note are left where they are, and a string too long to fit even on its own line stays too long, since `pkl format` does not split a literal either.
