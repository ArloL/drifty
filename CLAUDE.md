...
- **A release asset's name is the installer's only input.** mise scores assets
  ...
  and would hand an arm64 machine an x64 build. The unversioned `drifty-macos`,
  `drifty-linux` and `drifty-windows.exe` assets stay only because the
  `ArloL/homebrew-tap` formula downloads those URLs.
